...
public class ProjectDownloadServiceImpl implements ProjectDownloadService {
    ...
    @Override
    public void downloadProjectAsZip(String projectPath, String downloadName,
                                     HttpServletResponse response) throws Throwable {
        // 1. 参数校验
        ThrowUtils.throwIf(StrUtil.isBlank(projectPath),
                ErrorCode.PARAMS_ERROR, "项目路径为空");
        File projectDir = new File(projectPath);
        ThrowUtils.throwIf(!projectDir.exists(), ErrorCode.PARAMS_ERROR,
                "项目路径不存在");
        ThrowUtils.throwIf(!projectDir.isDirectory(), ErrorCode.PARAMS_ERROR,
                "非法项目路径");

        // 2. 过滤器
        FileFilter filter = pathName -> isPathAllowed(projectDir.toPath(),
                pathName.toPath());
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "压缩文件失败");
        }
    }
