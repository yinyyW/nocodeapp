...
public class CodeFileSaverExecutor {
     ...
     * @return 保存的目录
     */
    public static File saveCode(CodeGenTypeEnum codeGenTypeEnum, Object result) {
        return switch (codeGenTypeEnum) {
            case CodeGenTypeEnum.HTML -> {
                yield htmlCodeFileSaverTemplate.saveCode((HtmlCodeResult) result);
            }
            case  CodeGenTypeEnum.MULTI_FILE -> {
                yield multiFileCodeSaverTemplate.saveCode((MultiFileCodeResult)  result);
            }
            default -> {
