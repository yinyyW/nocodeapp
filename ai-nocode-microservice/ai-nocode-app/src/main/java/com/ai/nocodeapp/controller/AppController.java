...
import com.ai.nocodeapp.common.constants.AppConstant;
...
import com.ai.nocodeapp.model.vo.AppVO;
import com.ai.nocodeapp.ratelimiter.annotation.RateLimit;
...
public class AppController {
    ...
    public BaseResponse<Boolean> deleteAppByAdmin(@RequestBody AppDeleteRequest appDeleteRequest, HttpServletRequest request) {
        ...
        User userInfo = InnerUserService.getLoginUser(request);
        boolean deleteResult = appService.deleteApp(appDeleteRequest.getId(),
                userInfo);
        ThrowUtils.throwIf(!deleteResult, ErrorCode.OPERATION_ERROR);
        return ResultUtils.success(true);
    ...
    @RateLimit(limitType = RateLimitType.USER, rateInterval = 60, rate = 5, message = "AI请求过于频繁，请稍后重试")
